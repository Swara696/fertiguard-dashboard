...
export default function Dashboard() {
  ...
  const t = translations[lang] || { subtitle: "Monitoring System", pipeline: "Pipeline Network", viewML: "ML Predictions" };
  ...
  return (
    <main className="min-h-screen p-4 sm:p-10 text-slate-100 selection:bg-cyan-500/30" style={{ background: 'radial-gradient(circle at top, #0f172a, #020617)' }}>
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-xs font-bold tracking-[0.2em] text-cyan-500 uppercase">System Active</span>
            </div>
            ...
            <div className="mt-4 flex items-center gap-4">
              <Badge tone={isClogged ? 'critical' : 'healthy'}>{isClogged ? 'Action Required' : 'Optimal'}</Badge>
            ...
            <div className="relative rounded-[2rem] bg-slate-950/80 backdrop-blur-2xl p-8 h-full">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-xl font-bold tracking-tight text-white">Sensory Node <span className="text-slate-500 font-light mx-2">|</span> Node 1</h3>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Metric label="pH Level" value={node1?.ph ?? "7.2"} unit="" glow="cyan" />
                <Metric label="Turbidity" value={node1?.turbidity ?? "0.4"} unit="NTU" glow="emerald" />
                <Metric label="Flow Rate" value={node1?.flow ?? "12"} unit="L/min" glow="indigo" />
            ...
            <div className="relative rounded-[2rem] bg-slate-950/80 backdrop-blur-2xl p-8 h-full">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-xl font-bold tracking-tight text-white">Mitigation Node <span className="text-slate-500 font-light mx-2">|</span> Node 2</h3>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Metric label="Flow Rate" value={node2?.flow ?? "10"} unit="L/min" glow="emerald" />
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="group hover:translate-y-[-4px] transition-transform duration-300">
            <ChartCard title="Soil Moisture Trends" hint="Aggregated Data">
              <LineChart labels={labels} data={soilData} accent="#10B981" unit="%" height={250} />
            </ChartCard>
          </div>
          <div className="group hover:translate-y-[-4px] transition-transform duration-300">
            <ChartCard title="Hydraulic Pressure" hint="PSI Monitoring">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-slate-900/40 border border-white/5 rounded-[2rem] p-8 backdrop-blur-sm">
            ...
            <p className="text-[10px] font-bold text-slate-500 mt-4 uppercase tracking-[0.2em] text-center">Branch {cloggedBranch} Critical Impedance Detected</p>
          </div>
...
function Metric({ label, value, unit, glow }) {
  ...
  return (
    <div className={`rounded-2xl p-5 bg-white/[0.03] border transition-all duration-500 ${glowMap[glow]}`}>
